...
```
아동학대 발생장소가 많이 일어나는 장소 상위 2곳은 가정내, 아동 가정 내로 발생하는 건수는 연도에 따라 증가하고 있으며, 위의 그래프를 통해 아동과 함께 사는 사람에 의해 학대가 일어남을 추측해 볼 수 있다.
```
### 5-3. 아동학대 유형별 현황
...
- 성학대는 어린 시절부터 지속적으로 피해를 당하는 경우가 많기 때문에,
자신의 의사를 표현할 수 있는 청소년기 때 비교적 늦은 시기에
신고 접수가 이루어짐
```
![Alt text](image-5.png)
```
- 4가지 유형 중 33가지 유형 신체학대, 정서학대, 방임 이 압도적으로 부모로부터 발생한 피해이므로 아동학대의 주된 가해자는 부모
...
```
55대 강력범죄인 살인, 강도, 성폭력, 절도, 폭행인 경우로 국한시켜 분석했으며, 연도별 범죄발생 건수와 아동학대 의심사례는 모두 0.8 이상의 상관계수 값을 가지므로 강한 양의 상관관계임을 알 수 있다.
```
...
|2021|0.92202|
```
앞의 두 가지 요인과 비교했을 때 가장 높은 상관계수 값을 가지며, 연도별 비경제활동인구 수와 아동학대 의심사례는 모두 0.8 이상의 상관계수 값을 가지므로 강한 양의 상관관계임을 알 수 있다.
```
```
